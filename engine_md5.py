...
def predict_pro(md5_hex, gate_key=""):
    ...
    return {
        "engine": "PRO-V10 SHA512/SHA3/BLAKE2 CHAOS MATRIX",
        "taixiu": result,
        "tx_conf": confidence,
        "chanle": chanle,
        "dice": f"{d1}-{d2}-{d3}",
        ...
        "hash_short": s[:8].upper() + "..." + s[-6:].upper(),
        "trend": trend,
        "details": ["SHA512 salt", "SHA3 anti-bias", "BLAKE2b cross-check", "Chaos resonance", "Mirror entropy", "Gate-specific matrix"]
    }
...
def predict_free(md5_hex, gate_key=""):
    ...
    return {
        ...
        "score": round(score, 2),
        "hash_short": s[:8].upper() + "..." + s[-6:].upper(),
        "details": ["Lite wave", "Simple XOR", "Nibble scan"]
    }
