...
def gates_by_mode(mode="md5"):
    ...
    for k, v in enabled_gates().items():
        if mode == "md5" and bool(v.get("md5", False)):
            ...
            # V99: tự kiểm tra API. Cổng lỗi sẽ khóa tạm khỏi menu API để user khỏi vào nhầm.
...
def predict_nomd5(gate="lc79", level="basic", history=None):
    ...
    if not ok:
        return {
            ...
            "advice_reason":"API chưa trả phiên thật, tạm khóa API cho cổng này."
        }
    ...
    # Seed dùng phiên kế tiếp, không dùng random/time => 5s load vẫn ổn định trong cùng phiên.
    ...
    return {
        ...
        "details":["API thật", "Không chậm phiên", "Live 5s", gate_name(gate)],
...
def format_md5_reply(gate, p):
    """Reply MD5 gọn đồng bộ với API, thêm dòng mã MD5 hiện tại."""
    result = p.get("taixiu", "ĐANG CHỜ")
    if result == "TÀI":
        line = "📈 Kết luận: <b>🅣 TÀI</b>"
    elif result == "XỈU":
...
def start_nomd5_live(bot_obj, chat_id, message_id, uid, gate, plan):
    """Cập nhật cùng 1 tin nhắn mỗi 5s, chạy lâu và không spam tin mới."""
    key = f"{chat_id}:{message_id}"
    ...
    def cb(c):
        ...
        if data in ("choose_gate", "choose_type"):
            ...
            b.edit_message_text(
                ...
                "2️⃣ <b>API</b>: bot tự lấy phiên/game API rồi dự đoán.\n\n"
