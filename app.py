...
import os, json, time, threading, secrets, hashlib, statistics, urllib.parse, random
...
def _backup_endpoint():
    """
    Admin chỉ cần nhập domain Infinity hoặc link backup.php:
    - https://domain.com
    - https://domain.com/
    - https://domain.com/backup.php
    Source sẽ tự đổi thành /backup.php nếu cần.
    """
    raw = str(cfg.get("backup_url", "")).strip()
    if not raw:
        return ""
    ...
    if raw.endswith("/"):
        raw += "backup.php"
    elif not raw.lower().endswith(".php"):
        raw += "/backup.php"
    return raw

def backup_payload():
    # Backup chỉ gồm config + database, không backup/khôi phục code hay engine_md5.py
    return {"time": now_str(), "config": cfg, "database": db}

def push_backup(reason="auto"):
    url = _backup_endpoint()
    if not url:
        return False, "Chưa cài domain backup"
    try:
        r = requests.post(url, json={"reason": reason, "payload": backup_payload()}, timeout=15)
...
def pull_backup():
    ...
    try:
        ...
        if isinstance(new_cfg, dict) and isinstance(new_db, dict):
            # Không để backup đè rỗng backup_url hiện tại nếu file cũ chưa có
            old_backup_url = cfg.get("backup_url", "")
            cfg.clear(); cfg.update(new_cfg)
            if not cfg.get("backup_url"):
                cfg["backup_url"] = old_backup_url
...
def free_key(uid):
    return datetime.now().strftime("%Y-%m-%d") + ":" + str(uid)

def free_left(uid):
    if not cfg.get("free_mode_enabled", False):
        return 0
    key = free_key(uid)
    used = int(db.setdefault("free_usage", {}).get(key, 0) or 0)
    return max(0, int(cfg.get("free_daily_limit", 5) or 5) - used)

def use_free(uid):
    key = free_key(uid)
    db.setdefault("free_usage", {})[key] = int(db.setdefault("free_usage", {}).get(key, 0) or 0) + 1
    save_db()
...
def predict_free(md5_hex: str, gate_key=""):
    if engine_predict_free:
        r = engine_predict_free(md5_hex, gate_key)
        if r:
            return r
    return legacy_predict_basic(md5_hex)

def predict_basic(md5_hex: str):
    if engine_predict_basic:
        r = engine_predict_basic(md5_hex)
        if r:
            return r
    return legacy_predict_basic(md5_hex)

def predict_pro(md5_hex: str, gate_key=""):
    if engine_predict_pro:
        r = engine_predict_pro(md5_hex, gate_key)
        if r:
            return r
    return legacy_predict_pro(md5_hex, gate_key)
    ...
    def cb(c):
        ...
        if data.startswith("buy_"):
            ...
            if pk not in cfg["plans"]:
                return
            p = cfg["plans"][pk]
            user_state[uid] = {"mode": "buy_hours", "plan": pk}
            b.edit_message_text(
    ...
    def allmsg(m):
        ...
        if len(text) == 32:
            ...
            if not has_active_plan(user):
                if not cfg.get("free_mode_enabled", False) or free_left(uid) <= 0:
            ...
            b.reply_to(
                ...
                f"{('🎁 Free còn: <b>' + str(free_left(uid)) + '</b> lượt hôm nay\n') if free_used else ''}\n"
...
def settings():
    ...
    if request.method == "POST":
        ...
        cfg["free_daily_limit"] = int(request.form.get("free_daily_limit",5) or 5)
    ...
    <div class="grid2">
      <div class="card"><h2>🤖 Bot & Giao diện</h2>
        ...
        <label>Group Chat ID hoặc @username nhận thông báo</label><input name="group_chat_id" value="{cfg.get('group_chat_id','')}" placeholder="-100xxxxxxxxxx hoặc @group_username">
        <label>Link group public</label><input name="group_link" value="{cfg.get('group_link','')}" placeholder="https://t.me/ten_group_public">
        <p class="mut">Bot phải được add vào group. Link public hoặc @username dùng được. Link invite dạng t.me/+... không tự đổi thành chat_id được.</p>
      </div>
      <div class="card"><h2>💳 VietQR</h2>
        <label>Tên ngân hàng</label><input name="bank_name_key" value="{cfg.get('bank_name_key','MSB')}" placeholder="VD: MSB, VCB, MBBANK, ACB"><p class="mut">Không cần nhập BIN nữa. Source tự đổi tên bank sang BIN VietQR.</p>
        ...
        <label>Tiền tố nội dung</label><input name="payment_note_prefix" value="{cfg.get('payment_note_prefix','NAP')}"><hr style="border-color:var(--bd)"><h2>☁️ Backup PHP Infinity</h2><label>Domain Infinity đã up backup.php</label><input name="backup_url" value="{cfg.get('backup_url','')}" placeholder="https://domain.com hoặc https://domain.com/backup.php"><label>Chu kỳ backup giây</label><input type="number" name="backup_interval_seconds" value="{cfg.get('backup_interval_seconds',120)}">
        <hr style="border-color:var(--bd)">
        <h2>🎁 Free & Bonus</h2>
        <label><input style="width:auto" type="checkbox" name="free_mode_enabled" {'checked' if cfg.get('free_mode_enabled') else ''}> Bật dự đoán free</label>
        <label>Lượt free mỗi ngày</label><input type="number" name="free_daily_limit" value="{cfg.get('free_daily_limit',5)}">
        <label><input style="width:auto" type="checkbox" name="deposit_bonus_enabled" {'checked' if cfg.get('deposit_bonus_enabled') else ''}> Bật bonus nạp tiền</label>
        ...
        <label><input style="width:auto" type="checkbox" name="purchase_bonus_enabled" {'checked' if cfg.get('purchase_bonus_enabled') else ''}> Bật random bonus khi mua gói</label>
...
def backup_page():
    ...
    <div class='grid2'>
      <div class='card'>
        <h2>Trạng thái</h2>
        <p>Domain/URL: <code>{cfg.get('backup_url','')}</code></p>
      ...
      <div class='card'>
        <h2>Hướng dẫn</h2>
        <p class='mut'>Up file backup.php lên Infinity. Điền domain Infinity ở Cài đặt. Bot sẽ tự kéo backup lúc start và tự đẩy backup theo chu kỳ.</p>
      </div>
    </div>
    """
