...
def load_admin_algorithm():
    ...
    safe_builtins = {
        ...
        "bool": bool, "pow": pow, "__import__": __import__
    }
...
def recent_history_for_gate(gate_key="", limit=500):
    ...
    for item in reversed(db.get("predictions", [])):
        if gate_key and str(item.get("gate", "")).lower().strip() not in ("", gate_key):
            continue
        r = str(item.get("real") or item.get("result") or "").upper()
...
def apply_tier_policy(res, public_level="basic"):
    ...
    res["engine"] = f"{res.get('engine','ENGINE')} · {public_level.upper()}={res['tier_label']}"
    ...
    for lv in ("free", "basic", "pro"):
        ...
        <div class='tier-card card'>
          ...
          <select name='{lv}_mode'>
            ...
            {option(mode,'royal','SIÊU XỊN / Royal Pro')}
          </select>
    ...
    <div class='grid2'>
      ...
      <div class='card'>
        ...
        <form method='post'>
          ...
          <label>Game</label><select name='test_game'><option value='hitclub'>hitclub</option><option value='lc79'>lc79</option><option value='bet'>bet</option><option value='auto'>auto</option></select>
