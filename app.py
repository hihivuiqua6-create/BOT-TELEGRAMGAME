...
def _extract_nomd5_payload(data, spec):
    """Chuẩn hóa JSON API thành period/total/dice/result/confidence."""
    items = _flatten_recent_items(data)
    candidates = items if items else ([data] if isinstance(data, dict) else [])
    # ưu tiên item mới nhất: nhiều API đã sort mới nhất đầu list
    for item in candidates[:8]:
        ...
        if dice and total in (None, ""):
            total = sum(dice)
        result = _norm_tx(pred)
        if not result:
            result = _tx_from_total(total)
        if period:
            return {
                "period": str(period),
                "total": int(float(total)) if str(total or "").replace(".", "", 1).isdigit() else (sum(dice) if dice else None),
                "dice": dice,
                "prediction": result,
...
def predict_nomd5(gate="lc79", level="basic", history=None):
    ...
    hb = _history_bits(history)[-120:] if history else []
    if not ok:
        return {
    ...
    # Seed ổn định: cùng phiên/cổng/gói/history => cùng kết quả, không dùng random/time.
    seed = f"NO-MD5-API-V100|{gate}|{level}|{period}|{api_result}|{api_conf}|{api_total}|{''.join(map(str, hb))}"
    h = hashlib.sha256(seed.encode()).hexdigest()
    b = [int(h[i:i+2],16) for i in range(0,64,2)]

    # Nếu API có kết quả/tổng thật thì dùng làm nền dữ liệu. Nếu API chỉ có phiên, engine tự suy luận.
    result = api_result if api_result in ("TÀI", "XỈU") else ("TÀI" if b[0] >= 128 else "XỈU")
    total = None
    try:
        total = int(api_total) if api_total not in (None, "") else None
    except Exception:
        total = None
    dice_arr = api_dice if api_dice else (_dice_from_total_seed(total, seed) if total else None)
    if not dice_arr:
        ...
        while result == "XỈU" and total > 10 and guard < 12:
            ...
            total = sum(dice_arr); guard += 1
    if not result:
        result = "TÀI" if total and total >= 11 else "XỈU"

    runs = _runs(hb) if hb else []
    last_run = runs[-1][1] if runs else 0
    changes = sum(hb[i]!=hb[i-1] for i in range(1,len(hb))) if hb else 0
    hist_bonus = min(9, len(hb)//12)
    gate_boost = {"hitclub":3, "betvip":3, "lc79":2, "sunwin":2, "b52":2}.get(gate,0)
    level_boost = {"free":-6, "basic":0, "pro":4}.get(level,0)
    agreement = 50 + (stable_int(seed, "vote", changes, last_run, mod=1600) / 100.0)  # 50-66
    conf_cap = 68 if level == "free" else 86 if level == "basic" else 94
    conf_floor = 49 if level == "free" else 53 if level == "basic" else 57
    conf = int(_clamp(round((api_conf*0.58) + (agreement*0.32) + hist_bonus + gate_boost + level_boost), conf_floor, conf_cap))
    if len(hb) < 6:
        conf = min(conf, 64 if level != "pro" else 68)
    if result == "TÀI":
        prob_t, prob_x = conf, 100-conf
    else:
        prob_x, prob_t = conf, 100-conf
    score = round((prob_t if result == "TÀI" else prob_x) + stable_int(seed, "score", mod=900)/100, 2)
    return {
        ...
        "chanle":"CHẴN" if isinstance(total,int) and total % 2 == 0 else "LẺ", "score":score, "trend":"API phiên thật + engine V100",
        "advice":"✅ NÊN THEO" if conf >= 70 else "⚖️ CÂN NHẮC" if conf >= 58 else "⏸️ BỎ QUA",
        ...
        "details":["API thật", "Phiên chuẩn", "V100", gate_name(gate)],
        "history_len":len(hb), "vote_agree":round(agreement/10,1), "hash_short":"NO-MD5",
...
def start_nomd5_live(bot_obj, chat_id, message_id, uid, gate, plan):
    """Cập nhật cùng 1 tin nhắn mỗi 5s, không spam tin mới."""
    ...
    def worker():
        ...
        for _ in range(120):
            ...
            try:
                ...
                if text != last_text:
                    bot_obj.edit_message_text(text, chat_id, message_id, reply_markup=kb_home())
                    last_text = text
            except Exception as e:
