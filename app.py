...
import os, json, time, threading, secrets, hashlib, statistics, urllib.parse, random, traceback, html, re
...
# Chuẩn hoá danh sách cổng: config cũ thường thiếu md5/nomd5 nên No MD5 chỉ hiện 1 cổng.
def ensure_gate_config():
    ...
    # Loại theo yêu cầu: B52 và SunWin không nằm ở tab MD5, chỉ nằm No MD5
...
def gates_by_mode(mode="md5"):
    ...
    for k, v in enabled_gates().items():
        if mode == "md5" and bool(v.get("md5", False)):
            ...
            # V99: tự kiểm tra API. Cổng lỗi sẽ khóa tạm khỏi menu No MD5 để user khỏi vào nhầm.
            if is_nomd5_gate_available(k):
                out[k] = v
    return out
...
NOMD5_API_MAP = {
    ...
    # No MD5 chỉ dùng các cổng thật có API phiên.
    "lc79": {
        "urls": [
...
def is_nomd5_gate_available(gate):
    """Health check nhẹ cho menu No MD5. Cache 90s để không spam API."""
...
def _next_period_value(period):
    """No MD5 dự đoán phiên kế tiếp: API thường trả phiên vừa chốt nên cần +1."""
...
def _fetch_nomd5_api(gate, health_only=False):
    ...
    if not spec:
        return False, {"error": "Cổng chưa có API No MD5"}
    ttl = 0.8
    now = time.time()
    ck = _nomd5_cache.get(gate)
    if ck and now - ck.get("time", 0) < ttl:
        return True, ck["data"]
    urls = spec.get("urls") or ([spec.get("url")] if spec.get("url") else [])
    errors=[]
    headers={
        "User-Agent":"Mozilla/5.0 KingBot-NoMD5/V100 RealAPI",
...
def _nomd5_markov_vote(bits):
    """Vote theo lịch sử phiên thật, bits[0] là phiên mới nhất."""
    if not bits:
        return 0, 0.0
    seq = list(reversed(bits))  # cũ -> mới
    votes=[]
    for k, w in ((1, 9), (2, 13), (3, 17), (4, 21)):
        if len(seq) <= k + 2:
            continue
        cur = tuple(seq[-k:])
        t=x=0
        ...
        if t+x:
            votes.append((1 if t >= x else 0, abs(t-x)/(t+x), w))
    if not votes:
        return 0, 0.0
    score=sum((1 if bit else -1)*strength*w for bit,strength,w in votes)
    total=sum(w for _,_,w in votes)
    return (1 if score >= 0 else -1), min(1.0, abs(score)/max(1,total))


def _nomd5_pattern_vote(bits):
    if not bits:
        return 0, 0.0
    # bits[0] mới nhất. Dự đoán tiếp theo theo các dạng cầu phổ biến.
    seq = list(reversed(bits))
    last = seq[-1]
    # bệt dài: ưu tiên theo tiếp, nhưng nếu quá dài thì cảnh báo bẻ
    runs = _runs(seq)
    last_run = runs[-1][1] if runs else 1
    if last_run >= 5:
        return (-1 if last == 1 else 1), 0.58  # cầu rất dài dễ bẻ
    if last_run >= 3:
        return (1 if last == 1 else -1), 0.52
    # đảo 1-1
    if len(seq) >= 6 and all(seq[-i] != seq[-i-1] for i in range(1,6)):
        return (-1 if last == 1 else 1), 0.70
    # 2-2
    if len(seq) >= 8 and seq[-8:] in ([1,1,0,0,1,1,0,0], [0,0,1,1,0,0,1,1]):
        return (1 if seq[-1] == seq[-2] else (-1 if last == 1 else 1)), 0.62
    # 2-1 / 1-2 nhẹ
    tail = seq[-6:] if len(seq) >= 6 else []
    if tail in ([1,1,0,1,1,0], [0,0,1,0,0,1]):
        return (1 if last == 1 else -1), 0.57
    return 0, 0.0
...
def predict_nomd5(gate="lc79", level="basic", history=None):
    ...
    external_hb = _history_bits(history)[-120:] if history else []
    if not ok:
        return {
            ...
            "advice_reason":"API chưa trả phiên thật, tạm khóa No MD5 cho cổng này."
        }

    last_period = str(api.get("period") or "ĐANG-CẬP-NHẬT")
    period = str(api.get("next_period") or _next_period_value(last_period))
    api_conf = _num_conf(api.get("confidence"), 55)
    api_total = api.get("total")
    api_dice = api.get("dice")
    api_hb = list(api.get("history_bits") or [])
    hb = (api_hb + external_hb)[-160:]

    # Seed dùng phiên kế tiếp, không dùng random/time => 1s load vẫn ổn định trong cùng phiên.
    seed = f"NO-MD5-LIVE-V101|{gate}|{level}|next={period}|last={last_period}|hist={''.join(map(str,hb[:80]))}"
    h = hashlib.sha256(seed.encode()).hexdigest()
    b = [int(h[i:i+2],16) for i in range(0,64,2)]

    mv, ms = _nomd5_markov_vote(hb)
    pv, ps = _nomd5_pattern_vote(hb)
    hash_vote = 1 if (b[0] + b[7] + b[19]) % 100 >= 50 else -1
    gate_bias = {"hitclub": 2, "betvip": 1, "lc79": 0, "sunwin": 0, "b52": 0}.get(gate, 0)
    level_bias = {"free": -2, "basic": 0, "pro": 2}.get(level, 0)
    vote_score = hash_vote*0.32 + mv*(0.45+ms) + pv*(0.38+ps) + (gate_bias+level_bias)/20
    result = "TÀI" if vote_score >= 0 else "XỈU"

    # Dice là bộ mô phỏng cho phiên kế tiếp, không lấy nguyên dice phiên vừa chốt để khỏi chậm 1 phiên.
    if result == "TÀI":
        total = 11 + stable_int(seed, "totalT", mod=8)
    else:
        total = 3 + stable_int(seed, "totalX", mod=8)
    dice_arr = _dice_from_total_seed(total, seed) or [b[1]%6+1, b[5]%6+1, b[9]%6+1]
    total = sum(dice_arr)
    # Bảo đảm tổng khớp kết luận.
    guard=0
    ...
    hist_bonus = min(12, len(hb)//10)
    agreement = min(0.98, abs(vote_score))
    conf_cap = 68 if level == "free" else 86 if level == "basic" else 94
    conf_floor = 49 if level == "free" else 53 if level == "basic" else 57
    base = 51 + int(agreement * 23) + hist_bonus + gate_bias + level_bias
    if len(api_hb) < 5:
        base -= 6
    conf = int(_clamp(base, conf_floor, conf_cap))
    # Không bơm ảo khi ít lịch sử thật.
    if len(api_hb) < 6:
        conf = min(conf, 62 if level != "pro" else 66)

    if result == "TÀI":
        prob_t, prob_x = conf, 100-conf
    else:
        prob_x, prob_t = conf, 100-conf
    score = round((prob_t if result == "TÀI" else prob_x) + stable_int(seed, "score", mod=700)/100, 2)
    return {
        "engine":"Bản Nâng Cấp New", "game":gate.upper(), "period":period, "last_period":last_period,
        "taixiu":result, "tx_conf":conf,
        "prob_tai":f"{prob_t}%", "prob_xiu":f"{prob_x}%", "dice":"-".join(map(str,dice_arr[:3])), "total":total,
        "chanle":"CHẴN" if isinstance(total,int) and total % 2 == 0 else "LẺ", "score":score,
        "trend":"API phiên thật + dự đoán phiên kế tiếp", "advice":"✅ NÊN THEO" if conf >= 70 else "⚖️ CÂN NHẮC" if conf >= 58 else "⏸️ BỎ QUA",
        ...
        "details":["API thật", "Không chậm phiên", "Live 1s", gate_name(gate)],
        "history_len":len(api_hb), "vote_agree":round(agreement*10,1), "hash_short":"NO-MD5",
        "advice_reason":"Nên theo khi cầu đẹp, tín hiệu sạch và quản lý vốn hợp lý."
    }

def format_nomd5_reply(gate, p):
    """Reply No MD5 gọn theo yêu cầu."""
    result = p.get("taixiu", "ĐANG CHỜ")
    if result == "TÀI":
    ...
    return (
        ...
        f"{line}\n\n"
        f"📊 Độ tin cậy thực chiến: <b>{p.get('tx_conf')}%</b>\n\n"
...
def start_nomd5_live(bot_obj, chat_id, message_id, uid, gate, plan):
    """Cập nhật cùng 1 tin nhắn mỗi 1s, không spam tin mới."""
    key = f"{chat_id}:{message_id}"
    _nomd5_live_jobs[key] = {"alive": True, "uid": str(uid), "gate": gate, "plan": plan, "started": time.time()}
    def worker():
        last_text = None
        # chạy tối đa 10 phút/một màn hình; bấm lại sẽ tạo job mới.
        for _ in range(120):
            ...
            try:
                ...
                log(f"NoMD5 live update lỗi {gate}: {e}")
            time.sleep(1)
...
def build_bot():
    ...
    def cb(c):
        ...
        if data == "run_nomd5":
            gate = normalize_gate(db["users"][uid].get("active_gate"))
            if gate not in gates_by_mode("nomd5"):
                b.answer_callback_query(c.id, "Cổng này đang lỗi API/tạm khóa No MD5", show_alert=True)
                return
            ...
            db["predictions"].append({
                "user_id": uid, "username": username, "gate": gate, "plan": plan, "mode":"nomd5",
                "md5": "NO-MD5", "period": p.get("period"), "result": p.get("taixiu"),
                "confidence": p.get("tx_conf"), "time": now_str()
            })
            save_db()

            msg = format_nomd5_reply(gate, p)
            b.edit_message_text(msg, chat, mid, reply_markup=kb_home())
            # V100: giữ đúng một tin nhắn, 5 giây tự refresh phiên/kết quả trong đoạn chat này.
