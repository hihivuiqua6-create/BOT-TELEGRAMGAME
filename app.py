...
def build_bot():
    ...
    def allmsg(m):
        ...
        if len(text) == 32:
            ...
            trend = ("\n🧭 Bắt cầu: <b>" + p.get("trend", "") + "</b>") if p.get("trend") else ""
...
def web_500(e):
    log("WEB 500: " + str(e)[:500])
    try:
        return render_page("error", "<div class='top'><div class='title'>⚠️ Có lỗi server</div></div><div class='card'>Đã ghi log lỗi. Vào mục Logs để xem chi tiết, bot vẫn tiếp tục chạy nếu token đúng.</div>", "Lỗi server đã được bắt, không làm trắng trang."), 500
    except Exception:
