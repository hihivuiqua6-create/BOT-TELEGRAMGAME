...
DEFAULT_CONFIG = {
    ...
    "gates": {
        "lc79": {"name": "LC79", "enabled": True},
        "hitclub": {"name": "HitClub", "enabled": True},
        "betvip": {"name": "BetVip", "enabled": True}
    },
    "plans": {
...
def kb_home():
    ...
    kb.add(
        InlineKeyboardButton("🎮 Chọn Cổng Game", callback_data="choose_gate"),
...
def build_bot():
    ...
    def cb(c):
        ...
        if data == "account":
            txt = (
                ...
                f"🆔 ID: <code>{uid}</code>\n"
                f"💰 Số dư: <b>{money(user.get('balance',0))}</b>\n"
                f"🎮 Cổng: <b>{gate_name(user.get('active_gate')) if user.get('active_gate') else 'Chưa chọn'}</b>\n"
        ...
        if data == "choose_gate":
            ...
            for gk, gv in enabled_gates().items():
                kb.add(InlineKeyboardButton("🎮 " + gv.get("name", gk.upper()), callback_data=f"gate_{gk}"))
            kb.add(InlineKeyboardButton("🔙 Quay Lại", callback_data="back_home"))
        ...
        if data.startswith("gate_"):
            ...
            save_db()
            b.edit_message_text(f"✅ Đã chọn cổng: <b>{gate_name(gk)}</b>\n\nTiếp theo hãy mua gói để phân tích.", chat, mid, reply_markup=kb_home())
            return
        ...
        if data == "buy_tool":
            ...
            for pk, p in cfg["plans"].items():
                price = plan_price(pk)
                kb.add(InlineKeyboardButton(f"{'💎' if pk=='pro' else '⭐'} {p['name']} - {money(price)} / {p.get('hours',1)} giờ", callback_data=f"buy_{pk}"))
            kb.add(InlineKeyboardButton("🔙 Quay Lại", callback_data="back_home"))
            b.edit_message_text(
                f"🛒 <b>MUA GÓI PHÂN TÍCH</b>\n\n🎮 Cổng: <b>{gate_name(user.get('active_gate'))}</b>\n💰 Số dư: <b>{money(user.get('balance',0))}</b>",
        ...
        if data.startswith("buy_"):
            ...
            db["users"][uid]["plan_expire"] = int((datetime.now() + timedelta(hours=int(p.get("hours", 1)))).timestamp())
            db["users"][uid]["total_spent"] = int(db["users"][uid].get("total_spent", 0)) + price
            order = {"id": "BUY" + str(int(time.time())) + uid[-4:], "user_id": uid, "username": username, "gate": user.get("active_gate"), "plan": pk, "amount": price, "time": now_str()}
            db["purchases"].append(order)
            db["transactions"].append({"user_id": uid, "type": "purchase", "amount": -price, "plan": pk, "time": now_str()})
            save_db()
            notify_group(
                f"🛒 <b>ĐƠN MUA GÓI THÀNH CÔNG</b>\n"
                f"👤 User: <b>{mask_user(uid, username)}</b>\n"
                f"🎮 Cổng: <b>{gate_name(order['gate'])}</b>\n"
                f"📦 Gói: <b>{p['name']}</b>\n"
                f"💰 Giá: <b>{money(price)}</b>"
            )
            b.edit_message_text(
                f"✅ <b>MUA GÓI THÀNH CÔNG!</b>\n\n🎮 Cổng: <b>{gate_name(user.get('active_gate'))}</b>\n📦 Gói: <b>{p['name']}</b>\n⏳ Hạn: <b>{active_until_text(db['users'][uid])}</b>\n\nGửi MD5 để phân tích.",
    ...
    def allmsg(m):
        ...
        if state.get("mode") == "deposit_amount" and text:
            ...
            order = {"id": order_id, "user_id": uid, "username": username, "amount": amount, "note": note, "status": "pending", "time": now_str()}
            ...
            notify_group(f"💰 <b>CÓ ĐƠN NẠP MỚI</b>\n👤 User: <b>{mask_user(uid, username)}</b>\n💵 Số tiền: <b>{money(amount)}</b>\n🧾 Mã: <code>{order_id}</code>")
            return
        ...
        if len(text) == 32:
            if not has_active_plan(user):
                b.reply_to(m, "❌ Bạn cần mua gói trước mới phân tích được.\nBấm /start → Chọn cổng game → Mua gói.")
                return
...
def settings():
    ...
    for gk, gv in cfg["gates"].items():
        gates_html += f"<label>{gk.upper()} tên</label><input name='gate_{gk}_name' value='{gv.get('name','')}'><label><input style='width:auto' type='checkbox' name='gate_{gk}_enabled' {'checked' if gv.get('enabled') else ''}> Bật {gk.upper()}</label><br>"
    plans_html = ""
    for pk, p in cfg["plans"].items():
...
def deposits():
    ...
    for o in reversed(db["deposits"]):
        rows += f"<tr><td><code>{o.get('id')}</code></td><td>{mask_user(o.get('user_id'), o.get('username',''))}</td><td>{money(o.get('amount',0))}</td><td><span class='badge'>{o.get('status')}</span></td><td>{o.get('time')}</td><td>{'' if o.get('status')=='approved' else f'<a class=\"btn green\" href=\"/approve_deposit/{o.get('id')}\">Duyệt</a>'}</td></tr>"
    return render_page("deposits", f"<div class='top'><div class='title'>💰 Đơn nạp</div></div><div class='card'><table><tr><th>Mã</th><th>User</th><th>Số tiền</th><th>TT</th><th>Thời gian</th><th></th></tr>{rows}</table></div>", msg)
