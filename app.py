...
def push_backup(reason="auto"):
    ...
    try:
        r = requests.post(url, json={"secret": secret, "reason": reason, "payload": backup_payload()}, timeout=12)
...
def pull_backup():
    ...
    try:
        sep = "&" if "?" in url else "?"
        r = requests.get(url + sep + "secret=" + urllib.parse.quote(secret), timeout=12)
        if r.status_code != 200:
            return False, f"HTTP {r.status_code}: {r.text[:200]}"
        data = r.json()
...
def settings():
    ...
    <div class="grid2">
      ...
      <div class="card"><h2>💳 VietQR</h2>
        ...
        <label>Tiền tố nội dung</label><input name="payment_note_prefix" value="{cfg.get('payment_note_prefix','NAP')}"><hr style="border-color:var(--bd)"><h2>☁️ Backup PHP Infinity</h2><label>Backup URL PHP</label><input name="backup_url" value="{cfg.get('backup_url','')}" placeholder="https://domain.com/backup.php"><label>Backup Secret</label><input name="backup_secret" value="{cfg.get('backup_secret','')}"><label>Chu kỳ backup giây</label><input type="number" name="backup_interval_seconds" value="{cfg.get('backup_interval_seconds',120)}">
