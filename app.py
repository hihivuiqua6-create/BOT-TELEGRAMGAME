...
NOMD5_API_MAP = {
    # Lấy đúng phiên/kết quả từ source API user gửi trong sadd.zip. Không dùng 1.html/fake period nữa.
    "sunwin": {
        "url": "http://160.250.136.225:3001/prediction",
        "period_keys": ["phien_hien_tai", "phien", "Phien", "round", "session"],
        "predict_keys": ["Prediction", "du_doan", "prediction", "predict", "result"],
        "confidence_keys": ["Độ Tin Cậy", "do_tin_cay", "confidence", "ti_le", "rate"],
    },
    "lc79": {
        "url": "http://160.250.136.225:5001/api/lc79_tx?key=1",
        "period_keys": ["phien_hien_tai", "phien", "Phien", "round", "session"],
        "predict_keys": ["du_doan", "Prediction", "prediction", "predict", "result"],
        "confidence_keys": ["do_tin_cay", "Độ Tin Cậy", "confidence", "ti_le", "rate"],
    },
    "hitclub": {
        "url": "http://160.250.136.225:5002/hu",
        "period_keys": ["phien_hien_tai", "phien", "Phien", "round", "session"],
        "predict_keys": ["du_doan", "Prediction", "prediction", "predict", "result"],
        "confidence_keys": ["do_tin_cay", "Độ Tin Cậy", "confidence", "ti_le", "rate"],
    },
    "b52": {
        "url": "http://160.250.136.225:5004/b52",
        "period_keys": ["phien", "phien_hien_tai", "Phien", "round", "session"],
        "predict_keys": ["du_doan", "Prediction", "prediction", "predict", "result"],
        "confidence_keys": ["ti_le", "do_tin_cay", "Độ Tin Cậy", "confidence", "rate"],
    },
    "betvip": {
        "url": "http://160.250.136.225:5001/api/betvip_tx?key=1",
        "period_keys": ["phien_hien_tai", "phien", "Phien", "round", "session"],
        "predict_keys": ["du_doan", "Prediction", "prediction", "predict", "result"],
        "confidence_keys": ["do_tin_cay", "Độ Tin Cậy", "confidence", "ti_le", "rate"],
    },
...
def _fetch_nomd5_api(gate):
    ...
    if ck and now - ck.get("time", 0) < ttl:
        return True, ck["data"]
    try:
        r = requests.get(spec["url"], headers={"User-Agent":"KingBot-NoMD5/98", "Accept":"application/json,text/plain,*/*", "Connection":"close"}, timeout=8)
        if r.status_code != 200:
            return False, {"error": f"API HTTP {r.status_code}"}
        try:
            data = r.json()
        except Exception:
            txt = r.text.strip()
            try:
                ...
                return False, {"error": "API không trả JSON", "raw": txt[:160]}
        ...
        payload = {
            "raw": data,
            "period": str(period or "ĐANG-CẬP-NHẬT"),
...
def predict_nomd5(gate="lc79", level="basic", history=None):
    ...
    seed = f"NO-MD5-API-V98|{gate}|{level}|{period}|{api_result}|{api_conf}|{''.join(map(str, hb))}"
