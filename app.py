...
NOMD5_API_MAP = {
    # V99: dùng danh sách API primary + fallback từ source sadd.zip.
    # Nếu primary lỗi/timeout thì tự thử fallback; cổng lỗi sẽ bị khóa tạm khỏi menu No MD5.
    "sunwin": {
        "urls": [
            "http://160.250.136.225:3001/prediction",
            "https://sunwinsicbo-2r4h.onrender.com/sicbo"
        ],
        "period_keys": ["phien_hien_tai", "phien", "Phien", "round", "session", "current_session", "id", "issue"],
        "predict_keys": ["Prediction", "du_doan", "prediction", "predict", "result", "ket_qua", "taixiu"],
        "confidence_keys": ["Độ Tin Cậy", "do_tin_cay", "confidence", "ti_le", "rate", "percent"],
    },
    "lc79": {
        "urls": [
            "http://160.250.136.225:5001/api/lc79_tx?key=1",
            "https://laucua79-p7tw.onrender.com/taixiu"
        ],
        "period_keys": ["phien_hien_tai", "phien", "Phien", "round", "session", "current_session", "id", "issue"],
        "predict_keys": ["du_doan", "Prediction", "prediction", "predict", "result", "ket_qua", "taixiu"],
        "confidence_keys": ["do_tin_cay", "Độ Tin Cậy", "confidence", "ti_le", "rate", "percent"],
    },
    "hitclub": {
        "urls": [
            "http://160.250.136.225:5002/hu",
    ...
    "b52": {
        "urls": [
            "http://160.250.136.225:5004/b52",
            "https://b52-b74u.onrender.com/taixiu"
        ],
        "period_keys": ["phien", "phien_hien_tai", "Phien", "round", "session", "current_session", "id", "issue"],
        "predict_keys": ["du_doan", "Prediction", "prediction", "predict", "result", "ket_qua", "taixiu"],
        "confidence_keys": ["ti_le", "do_tin_cay", "Độ Tin Cậy", "confidence", "rate", "percent"],
    },
    "betvip": {
        "urls": [
            "http://160.250.136.225:5001/api/betvip_tx?key=1",
...
def _fetch_nomd5_api(gate, health_only=False):
    ...
    errors=[]
    headers={"User-Agent":"Mozilla/5.0 KingBot-NoMD5/99", "Accept":"application/json,text/plain,*/*", "Connection":"close"}
    for url in urls:
        if not url:
            continue
        try:
            # timeout thấp để Render không bị treo worker.
            ...
            try:
                ...
                try:
                    ...
                    continue
            period = _deep_find_value(data, spec["period_keys"])
            pred = _deep_find_value(data, spec["predict_keys"])
            conf = _deep_find_value(data, spec["confidence_keys"])
            # Nếu API chỉ có lịch sử, lấy item mới nhất.
            if (not period or not pred) and isinstance(data, list) and data:
                latest = data[0]
                period = period or _deep_find_value(latest, spec["period_keys"])
                pred = pred or _deep_find_value(latest, spec["predict_keys"])
                conf = conf or _deep_find_value(latest, spec["confidence_keys"])
            # Chỉ coi là OK khi có phiên thật. Kết quả có thể chưa có thì vẫn cho dự đoán bằng thuật toán nội bộ.
            if not period or str(period).upper().startswith("API"):
                errors.append(f"{url} -> thiếu phiên")
                continue
            payload = {
                "raw": data,
                "period": str(period),
                "prediction": _norm_tx(pred),
...
def predict_nomd5(gate="lc79", level="basic", history=None):
    ...
    if not ok:
        return {
            ...
            "trend":"Không lấy được phiên thật từ API", "advice":"⏸️ BỎ QUA", "stake_level":0,
            "risk":"CAO", "risk_emoji":"🔴", "details":["API lỗi", str(api.get("error", ""))[:60]],
            "history_len":len(hb), "vote_agree":0, "hash_short":"NO-MD5",
            "advice_reason":"API chưa trả phiên thật, không nên vào tiền."
        }
    period = str(api.get("period") or "ĐANG-CẬP-NHẬT")
    api_result = _norm_tx(api.get("prediction"))
    api_conf = _num_conf(api.get("confidence"), 55)
    seed = f"NO-MD5-API-V99|{gate}|{level}|{period}|{api_result}|{api_conf}|{''.join(map(str, hb))}"
    h = hashlib.sha256(seed.encode()).hexdigest()
    b = [int(h[i:i+2],16) for i in range(0,64,2)]
    # Ưu tiên kết quả API thật; thuật toán chỉ dùng để chấm rủi ro, vốn, % phụ trợ.
    result = api_result if api_result in ("TÀI", "XỈU") else ("TÀI" if b[0] >= 128 else "XỈU")
    runs = _runs(hb) if hb else []
    last_run = runs[-1][1] if runs else 0
    changes = sum(hb[i]!=hb[i-1] for i in range(1,len(hb))) if hb else 0
    distance_bonus = abs(api_conf - 50)
    hist_bonus = min(8, len(hb)//15)
    gate_boost = {"hitclub":2, "betvip":2, "lc79":1, "sunwin":1, "b52":1}.get(gate,0)
    conf_cap = 66 if level == "free" else 84 if level == "basic" else 92
    conf_floor = 50 if level == "free" else 54 if level == "basic" else 58
    conf = int(_clamp(api_conf + hist_bonus + gate_boost - (6 if len(hb) < 6 else 0), conf_floor, conf_cap))
    if result == "TÀI":
    ...
    while result == "XỈU" and total > 10 and guard < 12:
        ...
        total=d1+d2+d3; guard += 1
    risk_score = stable_int(seed,"risk",last_run,changes, mod=100)
    if conf >= 68 and len(hb) >= 10: risk_score = max(0, risk_score-15)
    risk = "THẤP" if risk_score < 32 else "TRUNG BÌNH" if risk_score < 68 else "CAO"
    risk_emoji = "🟢" if risk == "THẤP" else "🟡" if risk == "TRUNG BÌNH" else "🔴"
    advice = "✅ NÊN THEO" if conf >= (70 if level!="pro" else 66) and risk != "CAO" else "⚖️ CÂN NHẮC" if conf >= 58 else "⏸️ BỎ QUA"
    stake = 2 if advice.startswith("✅") else 1 if advice.startswith("⚖️") else 0
    trend = "API ra phiên thật + tín hiệu sạch" if advice.startswith("✅") else "Cầu chưa sạch / ưu tiên quản lý vốn" if advice.startswith("⚖️") else "Kèo yếu / nên bỏ qua"
    details = ["API thật", "Phiên chuẩn", "Markov", "Cầu bẻ", "Chu kỳ", gate_name(gate)]
    return {
        ...
        "chanle":"CHẴN" if isinstance(total,int) and total % 2 == 0 else "LẺ", "score":round(conf + (distance_bonus/4),2), "trend":trend,
