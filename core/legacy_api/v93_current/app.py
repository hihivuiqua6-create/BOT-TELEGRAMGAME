...
config = load_config()
TOKEN = config.get('bot_token','')  # ADMIN PANEL TOKEN ONLY
#  config.get('bot_token','')

if not TOKEN:
    print('❌ BOT TOKEN chưa được set trong Admin Panel (config.json)')
    time.sleep(3)
    raise SystemExit(1)
    print('WARNING: BOT_TOKEN missing from admin config')
