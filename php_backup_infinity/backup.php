<?php
// backup.php - Upload file này lên InfinityFree/shared host.
// Đổi secret dưới đây giống Backup Secret trong web admin Python.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (($j['secret'] ?? '') !== $SECRET) out(['ok'=>false,'error'=>'bad_secret']);
...
$secret = $_GET['secret'] ?? '';
if ($secret !== $SECRET) out(['ok'=>false,'error'=>'bad_secret']);
if (!file_exists($file)) out(['ok'=>false,'error'=>'no_backup_yet']);
