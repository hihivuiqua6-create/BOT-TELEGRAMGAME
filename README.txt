...
=== UPDATE 2026-06-13 ===
- Thêm nút “🎁 Free dự đoán” ngay dưới các gói ở menu chọn cổng/mua gói.
- Free Mode chạy theo thời gian trong admin: mở X giờ, hết giờ tự khóa.
- Có tùy chọn bắt user vào kênh trước khi dùng free. Bot sẽ check member bằng @username/chat_id kênh.
- Thuật toán đã tách khỏi file Python cố định: vào Admin -> Cài đặt -> “Thuật toán Admin HTML”, bật checkbox rồi dán code Python.
- Code thuật toán có thể dùng 1 trong các hàm:
  def predict(md5, gate='', level='basic')
  def predict_basic(md5)
  def predict_pro(md5, gate='')
  def predict_free(md5, gate='')
- Output tối thiểu cần return dict: {'taixiu':'TÀI' hoặc 'XỈU', 'tx_conf':70}. Các key khác như dice,total,engine,risk,trend là tùy chọn.
- Nếu thuật toán admin lỗi, bot tự fallback về engine_md5/engine cũ để không sập bot.

BẢN FIX 2026-06-13:
- Fix lỗi 500 ở /settings do placeholder f-string chứa dict chưa escape.
- Ưu tiên thuật toán Admin HTML. Nếu Admin code trống/lỗi thì dùng file algorithm_fallback_md5_400kb.txt.
- File algorithm_fallback_md5_400kb.txt là thuật toán dự phòng dạng TXT >400KB, bắt nhiều dạng cầu: bệt, đảo 1-1, 2-2, gãy, mirror, pair/song thủ, chẵn/lẻ, cao/thấp, sát nút, biên cực trị.
- Thêm trang Backup JS: tải backup_data.js và restore lại config/database từ JS.
- Kết quả bot hiện Bắt cầu/Xu hướng cho mọi gói.
